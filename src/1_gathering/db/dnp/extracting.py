...
def subsuck(data, element, parname=""):
    ...
    # that name
    data[cid]["Molecule_Formula"] = remove_tags(
        element.attrib.get("molecular-formula", "")
    )
    data[cid]["Molecule_Weight"] = element.attrib.get("min_molecular-weight", "")
    ...
    for child in element:
        ...
        if child.tag == "inchi":
            ...
            if (
                "Unrecognized" not in chi
                and "error" not in chi
                and "Uninterpretable" not in chi
            ):
...
for filename in glob.glob("../data/external/dbSource/dnp/31_2/xmlfiles/*.jar"):
    ...
    with zipfile.ZipFile(filename, "r") as zip:
        ...
        for file in zip.filelist:
            ...
            if int(100.0 * (count) / num_files) % 10 == 0 or count >= num_files - 1:
                print(
                    "\r - {}  {}".format(
                        filename.split("/")[2],
                        "#" * int(100.0 * (count + 1) / num_files),
                    ),
                    end="",
                )
            count += 1
        if status is True:
            ...
            print(outstatus(" NOT OK for {}/{} files ".format(fail, success), "red"))

print(
    '- Generating the CSV with the "full set" \
...
order_rows_small = ["CRC_Number", "Molecule_Name", "MolfileName"]
# This is just a file with less columns, not really useful
df[df.MolfileName.notnull()][order_rows_small].to_csv(
    "../data/external/dbSource/dnp/31_2/id_inchi.csv", index=False
)
...
try:
    ...
    olddf[~olddf.CRC_Number.isin(df.CRC_Number)][order_rows].to_csv(
        "removed_this_year.csv", index=False
    )
except:
    print("Weird error")
    pass
