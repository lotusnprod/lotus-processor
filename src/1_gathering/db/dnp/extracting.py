...
for filename in glob.glob('../data/external/dbSource/dnp/30_2/xmlfiles/*.jar'):
    count = 0
...
df[df.MolfileName.notnull()][order_rows].to_csv(
    "../data/external/dbSource/dnp/30_2/full_set.csv", index=False)
...
df[df.MolfileName.notnull()][order_rows_small].to_csv("../data/external/dbSource/dnp/30_2/id_inchi.csv",
                                                      index=False)
