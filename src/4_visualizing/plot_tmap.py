...
# Load lotus
df_meta = pd.read_csv("../data/processed/230106_frozen_metadata.csv.gz", sep=",")

# Fill NaN with 'Unknown'
values = {
    "organism_taxonomy_02kingdom": "Not attributed (Bacteria and Algae)",
    "organism_taxonomy_03phylum": "Unknown",
    "organism_taxonomy_04class": "Unknown",
    "organism_taxonomy_05order": "Unknown",
    "organism_taxonomy_06family": "Unknown",
    "organism_taxonomy_08genus": "Unknown",
    "organism_taxonomy_09species": "Unknown",
}
...
df_gb.rename(
    columns={"organism_taxonomy_02kingdom_count": "biosource_count"}, inplace=True
)
...
for col in taxo_cols:
    ...
    df_gb[col + "_nunique_cat"] = pd.cut(
        df_gb[col + "_nunique"], bins, labels=names, right=False
    )

names = ["1 unique source", "2-5 sources", "6-20 sources", ">20 sources"]
df_gb["biosource_count_cat"] = pd.cut(
    df_gb.biosource_count, bins, labels=names, right=False
)
...
structure_taxo = [
    "structure_taxonomy_npclassifier_01pathway_first",
...
for dic in dic_categories:
    ...
    dic_categories[dic]["data"], dic_categories[dic]["labels"] = Top_N_classes(
        dic_categories[dic]["Ncat"], data, labels
    )
...
simaroubaceae_data, simaroubaceae_labels = keep_only_given_class(
    ["Simaroubaceae"], data, labels
)

labels, data = Faerun.create_categories(
    df_gb["structure_taxonomy_npclassifier_01pathway_first"]
)
...
labels, data = Faerun.create_categories(
    df_gb["structure_taxonomy_npclassifier_02superclass_first"]
)
...
OTLkingdom_data, OTLkingdom_labels = keep_only_given_class(
    ["Archaeplastida", "Fungi", "Not attributed (Bacteria and Algae)", "Metazoa"],
    data,
    labels,
)
...
micro_big_4 = mcolors.ListedColormap(
    ["gainsboro", "#4E7705", "#9D654C", "#098BD9", "#7D3560"]
)
...
micro_big_4_3 = mcolors.ListedColormap(
    ["gainsboro", "#4E7705", "#6D9F06", "#9D654C", "#C17754", "#7D3560", "#A1527F"]
)
...
lf.store("../data/interim/tmap/230106_coords_lotus_2D_map4.dat")
with open("../data/interim/tmap/230106_lotus_2D_map4.pickle", "wb+") as f:
    pickle.dump(
...
pickle.dump(
    (x, y, s, t),
    open("../data/interim/tmap/230106_coords_lotus_2D_map4.dat", "wb+"),
    protocol=pickle.HIGHEST_PROTOCOL,
)
...
# lf = tm.LSHForest(1024, 64)
# lf.restore(
#     "../data/interim/tmap/220916_lotus_2D_map4.dat")

# hac, c_frac, ring_atom_frac, largest_ring_size = pickle.load(
#     open("../data/interim/tmap/220916_lotus_2D_map4.pickle", "rb")
# )
...
# t = list(t)
# pickle.dump(
#     (x, y, s, t), open("../data/interim/tmap/220916_coords_lotus_2D_map4.dat", "wb+"), protocol=pickle.HIGHEST_PROTOCOL
# )
...
x, y, s, t = pickle.load(
    open("../data/interim/tmap/230106_coords_lotus_2D_map4.dat", "rb")
)

hac, c_frac, ring_atom_frac, largest_ring_size = pickle.load(
    open("../data/interim/tmap/230106_lotus_2D_map4.pickle", "rb")
)
...
f.plot("../res/html/230106_lotus_map4_2D", template="smiles")
