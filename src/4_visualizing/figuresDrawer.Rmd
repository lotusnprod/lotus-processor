...
inhouseDb_structures_2plot <- openDbTripletsClean %>%
  distinct(inchikeySanitized, database) %>%
  group_by(database) %>%
  count(inchikeySanitized) %>%
  ungroup()
...
inhouseDb_structures_dis <- openDbTripletsClean %>%
...
inhouseDb_biologicalsource_2plot <- openDbTripletsClean %>%
  filter(!is.na(organismLowestTaxon)) %>%
  distinct(organismLowestTaxon, database) %>%
  group_by(database) %>%
  count(organismLowestTaxon) %>%
  ungroup()
...
inhouseDb_organism_dis <- openDbTripletsClean %>%
  filter(!is.na(organismLowestTaxon)) %>%
...
#   distinct(organismLowestTaxon, .keep_all = TRUE)
...
inhouseDbPairs_2plot <- openDbTripletsClean %>%
  ...
  distinct(inchikeySanitized,
           ...
           .keep_all = TRUE) %>%
  group_by(database) %>%
  count(inchikeySanitized, organismLowestTaxon) %>%
  ungroup()
...
inhouseDb_most_structures <- openDbTripletsClean %>%
  filter(!is.na(inchikeySanitized)) %>%
  count(inchikeySanitized) %>%
  arrange(desc(n))
...
inhouseDb_most_structures_2plot <- openDbTripletsClean %>%
  filter(inchikeySanitized == mostinchi) %>%
  distinct(inchikeySanitized, 
           organismLowestTaxon,
           database, 
           .keep_all = TRUE) %>%
  group_by(database, organismLowestTaxon) %>%
  count(inchikeySanitized) %>%
  ungroup()
...
inhouseDb_most_plant <- openDbTripletsClean %>%
  filter(!is.na(organismLowestTaxon)) %>%
  filter(organism_1_kingdom == "Plantae") %>%
  count(organismLowestTaxon) %>%
  arrange(desc(n))

mostplant <- as.character(inhouseDb_most_plant[inhouseDb_most_plant$organismLowestTaxon == "Panax ginseng", 1])

inhouseDb_most_biologicalsource_2plot <- inhouseDb %>%
  filter(organismLowestTaxon == mostplant) %>%
  distinct(inchikeySanitized, organismLowestTaxon,
           database, 
           .keep_all = TRUE) %>%
  group_by(organismLowestTaxon, database) %>%
  count(inchikeySanitized) %>%
  ungroup()
