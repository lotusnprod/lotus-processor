...
openDbTripletsGold <- read_delim(
  file = gzfile("../../data/interim/tables/4_analysed/gold.tsv"),
  col_types = cols(.default = "c"),
  delim = "\t",
  escape_double = FALSE,
  trim_ws = TRUE
...
organismMetadata <- read_delim(
  file = gzfile("../../data/interim/tables/3_curated/organismMetadata.tsv.gz"),
  delim = "\t",
  escape_double = FALSE,
  trim_ws = TRUE
  ) %>% 
...
structureMetadata <- read_delim(
  file = gzfile("../../data/interim/tables/3_curated/structureMetadata.tsv.gz"),
  delim = "\t",
  escape_double = FALSE,
  trim_ws = TRUE
  ) %>% 
...
referenceMetadata <- read_delim(
  file = gzfile("../../data/interim/tables/3_curated/referenceMetadata.tsv.gz"),
  delim = "\t",
  escape_double = FALSE,
  trim_ws = TRUE
  ) %>% 
...
inhouseDbTripletsGoldMeta <-
  left_join(inhouseDbTripletsGold, organismMetadata) %>%
...
mostplant <- as.character(inhouseDb_most_plant[inhouseDb_most_plant$organismCleaned == "Panax ginseng", 1])

inhouseDb_most_organism_2plot <- inhouseDb %>%
