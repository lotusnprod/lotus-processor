...
try:
    ...
    print(
        "Parsing gziped tab separated file "
        + "\n"
        + input_file_path
        + "with column: "
        + "\n"
...
df = pd.read_csv(myZip, sep="\t", encoding="utf-8", on_bad_lines="error")
...
df[inchi_column_header].replace(regex=True, inplace=True, to_replace=r'"', value=r"")
...
df.to_csv(ouput_file_path, sep="\t", index=False, compression="gzip", encoding="utf-8")
