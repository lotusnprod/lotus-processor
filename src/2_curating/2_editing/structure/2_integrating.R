...
translatedStructureTable <- translatedStructureTable |>
  ...
  dplyr::distinct(structureType, structureValue, structureTranslated) |>
...
bad_smiles <- c(
  "[OH-].[OH-].[OH-].[OH-].[OH-].[OH-].[V+6]",
  ## rdkit fails
  "Not Available",
  "Cc1ccc2c(c1)-n1-c(=O)/c=c\\c(=O)-n-2-c2cc(C)ccc2-1",
  ## in reality 'Cc1ccc2c(c1)-n1-c(=O)/c=c\c(=O)-n-2-c2cc(C)ccc2-1'
...
translatedStructureTableUnique <- translatedStructureTable |>
  dplyr::distinct(structureTranslated)
